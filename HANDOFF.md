...
- 2026-07-03（Opus）：**阶段 8 复制文本完成并合并 main（⭐全项目最关键路径）**（merge 9302613）。拆 8a（format.ts 纯函数管线）+ 8b（结果弹窗/剪贴板/语言快切）、同一分支 feat/p8-copy-text、一次合并。**我逐行亲审了 format.ts**（用户拍板的关键路径档位）：Type 组合固定序、按 selector 去重合并（mergeChanges+保最初 initialRect）、移动只输出初始→最终、Changes 表 `| cssProp | old | new |`、text/html/src 归 contentChanges（剥标签/data:mime，不塞原始内容）、describeSnap 与 snap.ts 语义对应、en/zh_CN 模板回退 en——全部正确、逐字对齐 §7.1+part37。43 条单测是真结构断言。8b copy-text.ts 剪贴板在点击手势内同步 writeText（关键正确性点，已核）、结果弹窗照搬 part37、语言快切局部不写 settings、settings.exportLang（en/zh_CN/auto）。终态：vitest 240、E2E 46×3 我亲跑稳定（copy-text ② 真实 readText 断言）。两处 V1 简化（规格容许内）：纯内容编辑归 "Style Modification"、Move Target 用坐标。CHANGELOG 补阶段 8 + Future 第 8 行打钩，**已 push origin/main（9302613）**。下一步：阶段 9 复制图片（§5 建议拆 9a 截图拼接/9b 叠加绘制；chrome.tabs.captureVisibleTab 限速 ≥600ms/屏、manifest 加 host_permissions `<all_urls>`、标注叠加程序化重绘、fixed 元素重复为已知妥协、总高钳 ≤14000px）。
