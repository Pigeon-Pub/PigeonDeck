...
test('② 造标注 → 点复制图片 → pd-image-output 弹窗出现且截图有非零宽度', async () => {
  ...
  // 等待弹窗，最长 50s（captureVisibleTab 可能慢）
  const panelAppeared = await waitForImagePanel(page, 50000);

  if (panelAppeared) {
    // 主断言：截图图片有非零 naturalWidth
    const naturalWidth = await page.evaluate(() => {
      const host = document.getElementById('pd-host');
      const img = host?.shadowRoot?.querySelector<HTMLImageElement>(
        '[data-testid="pd-image-shot"]'
      );
    ...
    console.warn(
      '[copy-image.spec] DEVIATION ②: pd-image-output panel did not appear within 50s. ' +
